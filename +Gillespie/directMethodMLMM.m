function [ t, x ] = directMethodMLMM( stoich_fcn, propensity_fcn, tspan, x0,...
                                  params, output_fcn, MAX_OUTPUT_LENGTH)
%DIRECTMETHOD Implementation of the Direct Method variant of the Gillespie algorithm
%   Usage:
%       [t, x] = directMethod( stoich_fcn, propensity_fcn, tspan, x0 )
%       [t, x] = directMethod( stoich_fcn, propensity_fcn, tspan, x0, params )
%       [t, x] = directMethod( stoich_fcn, propensity_fcn, tspan, x0, params, output_fcn )
%
%   Returns:
...
%
%       stoich_fcn:     Function of stoichiometries.
%                           next_state = f( current_state, rxn_num, params )

%                       Outputs the next state of the system.
%
...
num_positions = size(x0, 2);
T = zeros(MAX_OUTPUT_LENGTH, 1);
X = zeros(MAX_OUTPUT_LENGTH, num_positions);
T(1)     = tspan(1);
X(1,:)   = x0;
rxn_count = 1;
%% MAIN LOOP
while T(rxn_count) < tspan(2)        
    ...
    % Check that one species has taken over
    finalx = zeros(params.num_lanes,params.len_lane-4);
    x = X(1:rxn_count,:);
    for i = 0:(params.num_lanes-1)
        % S1 = x(rxn_count,(i*params.len_lane+3):((i+1)*params.len_lane-2))
        finalx(i+1,:) = x(rxn_count,(i*params.len_lane+3):((i+1)*params.len_lane-2));
    end
    if (sum(finalx,'all') == params.tot_num_cells) || (sum(finalx,'all') == 0)
        t = T(1:rxn_count);
        warning('SSA:ExtinctionSpecies',...
                'Simulation ended as one of the species has taken over.');
        return;
    end
    
    % Update time and carry out reaction mu
    T(rxn_count+1)   = T(rxn_count)   + tau;
    X(rxn_count+1,:) = stoich_fcn(X(rxn_count,:), mu, params);    
